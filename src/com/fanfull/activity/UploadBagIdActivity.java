 ...
 * 
 */
public class UploadBagIdActivity extends BaseActivity {
	/**
	 * 手持 批量入库
	 */
	...
	public void onClick(View v) {
		...
		case R.id.tv_scanbunch_activity_lock:
			if (!haveTaskRunning) {
				// buildLockPi();
				if (null == mBagIdList || 0 == mBagIdList.size()) {
					ToastUtil.showToastInCenter("没有袋子");
					return;
				}

				StringBuilder sb = new StringBuilder();
				sb.append(mBagIdList.size() + " ");

				Iterator<String> iterator = mBagIdList.iterator();
				...
				SocketConnet.getInstance().setRecieveListener(mRecieveListener);
				mDiaUtil.showProgressDialog();
				SocketConnet.getInstance().communication(500, sb.toString());
			} else {
				ToastUtil.showToastInCenter("有任务在进行");
			}
			break;
	...
	private class MyHandlerCallback implements Handler.Callback {
		...
		public boolean handleMessage(Message msg) {
			switch (msg.what) {
			case -1:
				mTvScannedNum.setText(String.valueOf(mBagIdList.size()));
				break;
			case -2:
				mDiaUtil.dismissProgressDialog();
				if (msg.obj.toString().startsWith("*500 01 ")) {
					ToastUtil.showToastInCenter("上传成功");
					mBagIdList.clear();
					mTvScannedNum.setText("-");
				} else {
					mDiaUtil.showDialog("上传失败");
				}
				break;
					...
					builder.setPositiveButton(MyContexts.TEXT_OK,
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog,
										int which) {
									UploadBagIdActivity.this.finish();
								}
							});
					builder.show();
				} else {
	...
	private class ScanTask implements Runnable {
		...
		public void run() {
			...
			while (!stopped) {
				...
				if (isUHFMode) {
					readSuccess = mUhfOp.fastReadEpc(UHFOperation.sEPC);
					if (readSuccess) {
						bagIdBuf = UHFOperation.sEPC;
					}
				} else {
				...
				final String bagID = ArrayUtils.bytes2HexString(bagIdBuf);
