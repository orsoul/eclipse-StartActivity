...
/**
 * @Description: 新锁 验封   InStoreNfcActivity
在正常验封之前需要检测如下东西：
1.检测是否已经完成封签

如果以上四点均通过后:
1.手持改写标志位，以及更新目录索引信息，以及交接信息，
2.手持向服务器发送封签事件码，封签码，以及袋Id到服务器，服务器验证是否可以正常验封。
 */
@SuppressLint("HandlerLeak")
public class CheckNfcCoverBagActivity extends BaseActivity {
	private final static String TAG = CheckNfcCoverBagActivity.class.getSimpleName();
	...
	// 该袋电池低，请勿再使用
	private final static int BAG_CANNOT_USE_BAG = 0x997;
	...
	private NetCheckTask mNetCheckTask;//进行验封
	...
	private String mBagTidcodeInNfc = null ;//存在NFC卡中的封签事件码
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/**第三 请求服务器校验是否合法封袋*/
		mNetCheckTask = new NetCheckTask();
		/**第四 更新袋锁，写标志位以及索引信息*/
	...
	Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case BAG_NO_INIT:
				...
				mBagInItTask.stop();
				new DialogUtil(CheckNfcCoverBagActivity.this).
				showNegativeReplyDialog(MyContexts.BAG_LOACK_NOT_INIT,MyContexts.TEXT_CANCEL);
				mVReadBagLock.setDoing(false);
				mBtnConfirm.setEnabled(true);
				break;
				
				/**在验封时,标志位不正常，此时需要指纹处理*/
			case READ_BAG_NO_COVER:
				...
				tp.createDialog(CheckNfcCoverBagActivity.this,"该袋未正常封签", 0);
				break;
				
			case BAG_CANNOT_USE_BAG:
				haveTaskRunning = false;
				SoundUtils.playFailedSound();
				mBagInItTask.stop();
				mVReadBagLock.setDoing(false);
				mBtnConfirm.setEnabled(true);
				new DialogUtil(CheckNfcCoverBagActivity.this)
						.showNegativeReplyDialog("该袋已受损,需要返厂维修！",
								MyContexts.TEXT_OK);
				break;	
			...
			case NET_CHECK_START:
				...
				break;
				/**开始上传验封任务，服务器反馈成功，得到扫描数量,但是没有更新UI*/
			...
			case CHECK_BARCODE_NET_FAILED:// 6
				...
				mStep = READ_BAGLOACK;//重新开始
			    new TipDialog()
					.createDialog(CheckNfcCoverBagActivity.this,0);
				break;
			...
			case BAG_WRITE_FLAG_FAILED:
				ThreadPoolFactory.getNormalPool().execute(new NetOpenFailedTask());
				mVUpdateBagLock.setDoing(false);
				haveTaskRunning = false;
				SoundUtils.playFailedSound();
				mWriteRfidTask.stop();
			...
			case CHECK_BARCODE_NET_SUCCESS:
				ToastUtil.showToastInCenter("本次花时:"+(System.currentTimeMillis() - timetmp)+"ms");
				haveTaskRunning = false;
				//等验封服务器完成
			...
			case SHOW_LOCK_RESULT:// 111
				...
				if("锁定批成功".equals(info)){
					...
					mTvLock.setText("已锁定");
					new TipDialog().createDialog(CheckNfcCoverBagActivity.this,1);
				}else {
					new TipDialog().createDialog(CheckNfcCoverBagActivity.this,0);
				}
				break;	
				/**锁定批失败*/
			case SHOW_LOCK_FAILED:
				new DialogUtil(CheckNfcCoverBagActivity.this).
				showPostiveReplyDialog(CheckNfcCoverBagActivity.this,MyContexts.LOCK_PI_TIMEOUT);
				break;
			}
	...
	private boolean setpower (int read,int write){
		...
		int dReadPower = SPUtils.getInt(getApplicationContext(),
				MyContexts.KEY_POWER_READ_INSTORE, read);
		int dWritePower = SPUtils.getInt(getApplicationContext(),
				MyContexts.KEY_POWER_WRITE_INSTORE, write);
		if (mUHFOp.setPower(dReadPower, dWritePower)) {
			LogsUtil.s("验封 设置 功率成功");
			return true;
		} else {
			LogsUtil.s("验封 设置 功率失败");
			return false;
		}
	}
	...
	/**
	 * 通过读取索引 确定这一次交接信息需要写的位置
	 * 记录该位置的是地址0x20的低两位，即其最后一个字节
	 * @param data  0x20 四字节数据
	...
	class ReadBagInItTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				LogsUtil.d(TAG, "j= "+j);
				if(j == -1)continue;//扫描失败
				if (j == 5) { 	// 已初始化
					
					/**读取密钥信息*/
					byte miyue  = (null == mNfcBagOp ? 0x00:mNfcBagOp.readMiyue());
					if(0xaa ==  (miyue & 0xFF) || 0x00 == miyue){
						SystemClock.sleep(50);
						continue;
					}
					int t = Integer.parseInt(ArrayUtils.bytes2HexString(
					...
					if ((byte) ArrayUtils.flagData[5 * ((mUid[1] & 0xff) % 10) + 2] 
							== (byte) (mFlag[0] ^ mCurrentMiyue)) {
						/**  正常 等于F3的情况 */
					}else if ((byte) ArrayUtils.flagData[5 * ((mUid[1] & 0xff) % 10) +4] 
							== (byte) (mFlag[0] ^ mCurrentMiyue)) { //F5
						LogsUtil.d(TAG, "电池电量低，请勿继续使用！");
						mHandler.sendEmptyMessage(BAG_CANNOT_USE_BAG);
						return;
					}else {
						/**  不等于F3 F5的情况 */
						LogsUtil.d(TAG, "没有正常封签操作 "+ArrayUtils.bytes2HexString(mFlag));
					...
					/**读取目录索引*/
					mIndexInfo = (null == mNfcBagOp ? null : mNfcBagOp.readIndexAddr());
					if(null == mIndexInfo){
						continue;
					}else if(mIndexInfo.length >3 && mIndexInfo[3] < 0x40){
						mIndexInfo = new byte[]{(byte) 0xff,(byte) 0xff,(byte) 0xff,0x40};
					}
					...
					/**读取袋中存放的封签事件码信息*/
					mBagTidcodeInNfc = mNfcBagOp.readBagBarCode();
					if(null == mBagTidcodeInNfc ){
	...
	class ReadAndInitEPCTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				mTid = mUHFOp.readTIDNogl(mUHFOp.mEPC);
				if (null == mTid || !ArrayUtils.bytes2HexString(mBqTidInNfc)
						.equals(ArrayUtils.bytes2HexString(mTid))
						|| !StaticString.bagid.equals(ArrayUtils.bytes2HexString(mUHFOp.mEPC))) {
					SystemClock.sleep(50);
					continue;
				}else {
					StaticString.tid = ArrayUtils.bytes2HexString(mTid);
					LogsUtil.d(TAG, "tid="+StaticString.tid);
				}
					
//				String readString = null;
//				readString = ArrayUtils.bytesToHexString(mUHFOp.readUHF(mUHFOp.mEPC, 1,0x20, 3, 0x00,30));
//				LogsUtil.d(TAG, "读出封签事件码:"+readString);
//				readString = readString == null?null:readString.substring(0, 60);
//				if(readString == null){
//					SystemClock.sleep(50);
//					continue;
//				}
//				StaticString.bagtidcode = readString;
	...
	class WriteRFIDTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				if(!ArrayUtils.bytes2HexString(mUid).equals(ArrayUtils.bytes2HexString(tmpID))){
				...
				/** 写交接信息到高频卡中*/
				if (! mNfcBagOp.writeExangeInfo(mExchangeStartAddr,ArrayUtils.hexString2Bytes(
						mExchangeInfo+ "E"))){
					//失败处理
					continue;
	...
	/**
	 * 第五步，发送验封信息。服务器验证是否可以正确封袋
	 * @author Administrator
	 *
	 */
	class NetCheckTask implements Runnable{ 
		...
		public void run() {
			...
			LogsUtil.d(TAG, "验封，返回:"+StaticString.information+"---len="+StaticString.information.length());
			if (pasrePiNumber()) {
