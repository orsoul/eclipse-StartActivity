 ...
 *
 */
public class DisEnableNFCEmptyBagActivity extends BaseActivity{
	private final static String TAG = DisEnableNFCEmptyBagActivity.class.getSimpleName();
		...
		private final static int BAG_DIS_ENABLE_LOCAL_UNENABLE=0;//该袋已经注销
		private final static int BAG_DIS_ENABLE_LOCAL_SUCCESS =     1;
		private final static int BAG_DIS_ENABLE_LOCAL_FAILED =     2;
		private final static int BAG_DIS_ENABLE_LOCAL_TIMEOUT =     3;
		
		private final static int BAG_DIS_ENABLE_NET_SUCCESS =     5;
		private final static int BAG_DIS_ENABLE_NET_FAILED =     6;
		private final static int BAG_DIS_ENABLE_NET_TIMEOUT =     7;
		
		private final static int BAG_WRITE_DIS_ENABLE_SUCCESS =     8;
		private final static int BAG_WRITE_DIS_ENABLE_FAILED =     9;
...
//			mBagOp.connection();
		...
		private void findViews() {
			...
			mBtnStart.setText("开始注销");

			mPlanScanNum = SPUtils.getInt(DisEnableNFCEmptyBagActivity.this,
		...
		Handler mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case SHOW_LOCK_RESULT:
					...
					new TipDialog().createDialog(DisEnableNFCEmptyBagActivity.this, 0);
					break;
				case READ_BAG_FAILED:
					...
					break;
				case BAG_DIS_ENABLE_LOCAL_SUCCESS://已经空袋已经注销
					haveTaskRunning = false;
				...
				case BAG_NO_INIT:
					...
					break;
				case BAG_DIS_ENABLE_LOCAL_FAILED:
				//	dealFailedInfo("读袋尚未注销！");
					haveTaskRunning = true;//请求服务器注销空袋
					ThreadPoolFactory.getNormalPool().execute(mNetTask);
					break;
					
				case BAG_DIS_ENABLE_NET_SUCCESS:
					haveTaskRunning = true;
					ThreadPoolFactory.getNormalPool().execute(mWriteRFIDTask);
					break;
				case BAG_DIS_ENABLE_NET_FAILED:
					haveTaskRunning = false;
					mWriteRFIDTask.writeError();
					ThreadPoolFactory.getNormalPool().execute(mWriteRFIDTask);
					dealFailedInfo(null);
					break;	
				case BAG_DIS_ENABLE_NET_TIMEOUT:
					haveTaskRunning = false;
					dealFailedInfo("验证注销码超时");
					mWriteRFIDTask.writeError();
					ThreadPoolFactory.getNormalPool().execute(mWriteRFIDTask);
					break;	
				case BAG_WRITE_DIS_ENABLE_SUCCESS:
					haveTaskRunning = false;
					LogsUtil.s("ok");
					mScannedNum++;
					// 判断 扫捆 是否完成
					...
					break;
				case BAG_WRITE_DIS_ENABLE_FAILED:
					LogsUtil.s("READ_BARCODE_BAD");
					dealFailedInfo("注销空袋失败");
					mBtnStart.setEnabled(true);
					break;
		...
		private void dealFailedInfo (String info){
			...
			mProgreeBar.setText("注销已停止!");
			mBtnStart.setText("重新注销");
		}
		
		
		public void createDialog( final String info,final int flag) {
			AlertDialog.Builder builder = new Builder(DisEnableNFCEmptyBagActivity.this);
			...
			builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface arg0, int arg1) {
					if (flag == 0) {
						...
						DisEnableNFCEmptyBagActivity.this.finish();
					}
				}
			});
			builder.create().show();
		...
		public Boolean pasrePiNumber() {
			...
			LogsUtil.d(TAG,"收到注销回复信息:"+StaticString.information+"----"+str);
	...
	class ReadBagTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				if (j == BAG_MATCH) { // 读取RFID卡第一块区的数据，看前2位是否为05(袋ID版本最新)，是则返回0
					// 已初始化
					mEnableCode = (null == mNfcBagOp ? mBagOp.readEnableBagCode():mNfcBagOp.readEnableBagCode());
					//解密后的注销码
					...
					Log.d(TAG, "mEnableCode2= "+tmpString);
					 if(tmpString != null && tmpString.equalsIgnoreCase("EEEEEEEE")){
						mHandler.sendEmptyMessage(BAG_DIS_ENABLE_LOCAL_SUCCESS);
						stoped = true;
						return;
					 }else if(tmpString != null){
						mHandler.sendEmptyMessage(BAG_DIS_ENABLE_LOCAL_FAILED);
						stoped = true;
						return;
					}else {
						mHandler.sendEmptyMessage(READ_BAG_FAILED);//zyp 20160311 应该是读出全是0 说明还没有注销
						stoped = true;
						return;
					}
			...
			stoped = true;
			LogsUtil.d("618 ReadBagTask RFID task end");
		}
	}
	class NetTask implements Runnable{
		...
		public void run() {
			// TODO Auto-generated method stub
			LogsUtil.d("NetTask  start");
			StaticString.information = null;// 判断回复信息之间清空之前信息
			SocketConnet.getInstance().communication(883);//
//			mHandler.sendEmptyMessage(BAG_DIS_ENABLE_NET_SUCCESS);
			...
			if (ReplyParser.waitReply()) {
				...
				if (pasrePiNumber()) {
					...
					mHandler.sendEmptyMessage(BAG_DIS_ENABLE_NET_FAILED);
				}
			} else {// 超时回复之后就返回上层 收不到信息就返回上层页面
				mHandler.sendEmptyMessage(BAG_DIS_ENABLE_NET_TIMEOUT);
			}
	...
	class WriteRFIDTask implements Runnable {
		...
		public void run() {
			...
			stoped = false;
			int count = 0;
			final int TIMES = 20;// 读取 EPC 的次数
			count = 0;
			byte tt[] = new byte[]{(byte)0xee,(byte)0xee,(byte)0xee,(byte)0xee};
				
			while (!stoped) {
				if (TIMES < ++count) {
					stoped = true;
					mHandler.sendEmptyMessage(BAG_WRITE_DIS_ENABLE_FAILED);
					break;
				}
				if(mNfcBagOp == null) {
					if (mBagOp.writeBlock(10, ArrayUtils.encryption(tt,mUid))) { //写入袋中时，是通过uid进行加密的
						// 写入注销码 成功
						LogsUtil.s(" 写入注销码 成功");
						stoped = true;
						mHandler.sendEmptyMessage(BAG_WRITE_DIS_ENABLE_SUCCESS);
						break;
					}
				}else {
