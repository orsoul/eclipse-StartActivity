...
/**
 * @Description: 新锁 出入库 在正常出入库之前需要检测如下东西： 1.检测是否已经完成封签
 * 
...
public class IOStoreNfcActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// mTypeOperation = intent.getIntExtra(MyContexts.KEY_OPERATION_TYPE,
		// 0);
		mTypeOperation = intent.getIntExtra(TYPE_OP.KEY_TYPE,
				TYPE_OP.IN_STORE_HAND);
	...
	Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case SHOW_LOCK_FAILED:
				new DialogUtil(IOStoreNfcActivity.this).showPostiveReplyDialog(
						IOStoreNfcActivity.this, MyContexts.LOCK_PI_TIMEOUT);
				break;
			}
	...
	class ReadAndInitEPCTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				boolean encrypt = AESCoder.myEncrypt(eventCode, mBqTidInNfc,
						false);
		...
		public void run() {
			...
			if (TYPE_OP.IN_STORE_HAND == mTypeOperation) {
				SocketConnet.getInstance().communication(
						SendTask.CODE_IN_STORE_UPLOAD_BAG_INFO);
			} else {
				SocketConnet.getInstance().communication(
						SendTask.CODE_OUT_STORE_UPLOAD_BAG_INFO);
			}
			if (!ReplyParser.waitReply()) {
				mHandler.sendEmptyMessage(CHECK_BARCODE_NET_TIMEOUT);
				return;
			}
			LogsUtil.d(TAG, "出入库，返回:" + StaticString.information + "---len="
		...
		public void run() {
			...
			while (!stoped) {
				...
				if (!mNfcBagOp.writeExangeInfo(mExchangeStartAddr,
						ArrayUtils.hexString2Bytes(getExchangeInfo()))) {
					// 失败处理
					continue;
				}
