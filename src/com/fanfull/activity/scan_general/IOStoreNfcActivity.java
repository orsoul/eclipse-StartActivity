...
2.手持向服务器发送封签事件码，封签码，以及袋Id到服务器，服务器验证是否可以正常出入库。
 */
@SuppressLint("HandlerLeak")
public class IOStoreNfcActivity extends BaseActivity {
	...
	private int mTypeOperation = 0;// 当前activity的类型
	...
	private NetInOutStoreTask mNetInOutStoreTask;//进行出入库
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mUHFOp.open(false) < 0) {
			ToastUtil.showToastInCenter(getResources().getString(R.string.text_init_uhf_failed));
			return;
		}
...
//		mTypeOperation = intent.getIntExtra(MyContexts.KEY_OPERATION_TYPE, 0);
		mTypeOperation = intent.getIntExtra(TYPE_OP.KEY_TYPE, TYPE_OP.IN_STORE_HAND);
		LogsUtil.d(TAG, "mTypeOperation : " + mTypeOperation);
...
//		mBagOp.connection();
		...
		/**第三 请求服务器校验是否合法封袋*/
		mNetInOutStoreTask = new NetInOutStoreTask();
		/**第四 更新袋锁，写标志位以及索引信息*/
	...
	protected void findView() {
		...
		if (TYPE_OP.IN_STORE_HAND == mTypeOperation) {
	...
	Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case BAG_NO_INIT:
				...
				mBagInItTask.stop();
				new DialogUtil(IOStoreNfcActivity.this).
			...
			case READ_BAG_NO_COVER:
				...
				tp.createDialog(IOStoreNfcActivity.this,"该袋未正常封签", 0);
				break;
			...
			case NET_CHECK_START:
				...
				ThreadPoolFactory.getNormalPool().execute(mNetInOutStoreTask);
				break;
				/**开始上传出入库任务，服务器反馈成功，得到扫描数量,但是没有更新UI*/
			...
			case CHECK_BARCODE_NET_FAILED:// 6
				...
				mStep = READ_BAGLOACK;//重新开始
			    new TipDialog()
					.createDialog(IOStoreNfcActivity.this,0);
				break;
			...
			case SHOW_LOCK_RESULT:// 111
				...
				if("锁定批成功".equals(info)){
					...
					mTvLock.setText("已锁定");
					new TipDialog().createDialog(IOStoreNfcActivity.this,1);
				}else {
					new TipDialog().createDialog(IOStoreNfcActivity.this,0);
				}
				break;	
				/**锁定批失败*/
			case SHOW_LOCK_FAILED:
				new DialogUtil(IOStoreNfcActivity.this).
				showPostiveReplyDialog(IOStoreNfcActivity.this,MyContexts.LOCK_PI_TIMEOUT);
				break;
			}
	...
	/**
	 * 获得交接信息的机构号，时间以及，功能标识
	 * @return 交接信息
	 */
	private String getExchangeInfo (){
		if (TYPE_OP.IN_STORE_HAND == mTypeOperation) {
			return mExchangeInfo+"B";
		} else {
			return mExchangeInfo+"C";
		}
	}
...
//		bagop.close();
	...
	class ReadAndInitEPCTask implements Runnable {
		...
		public void run() {
			...
			while (!stoped) {
				...
				if (null == mUHFOp.readUHFInTime(UHFOperation.MB_TID, 0x03, 6, 1000, mBqTidInNfc, UHFOperation.MB_TID, 0x03)) {
					continue;
				}
	...
	/**
	 * 第五步，发送出入库信息。服务器验证是否可以正确封袋
	 * @author Administrator
	 *
	 */
	class NetInOutStoreTask implements Runnable{ 
		...
		public void run() {
			...
			if (TYPE_OP.IN_STORE_HAND == mTypeOperation) {
				SocketConnet.getInstance().communication(SendTask.CODE_IN_STORE_UPLOAD_BAG_INFO);
			} else {
			...
			LogsUtil.d(TAG, "出入库，返回:"+StaticString.information+"---len="+StaticString.information.length());
			if (pasrePiNumber()) {
